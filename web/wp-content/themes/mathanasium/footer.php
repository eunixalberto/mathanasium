<?php
/**
 * The template for displaying the footer.
 *
 * @link https://developer.wordpress.org/themes/basics/template-files/#template-partials
 *
 * @package BootstrapFast
 */

?>
			</div><!-- #contennt -->
		</div><!-- .row -->
	</div><!-- .container -->
	<footer id="colophon" class="site-footer <?php echo esc_attr( bootstrapfast_container_type() ) ?>" role="contentinfo">
		<div class="row">
			<div class="col-md-12">
				<div class="footercontainer">
					<?php
					if ( have_rows( 'footer_elements' ) ) {
						while ( have_rows( 'footer_elements' ) ) {
							the_row();
							?>
							<section id="custom_html-3" class="widget_text footerbar widget widget_custom_html">
								<div class="textwidget custom-html-widget">
									<p class="foot1"><?php echo esc_html( get_sub_field( 'foot1' ) ); ?></p>
									<p class="foot2"><?php echo esc_html( get_sub_field( 'foot2' ) ); ?></p>
									<p class="foot3"><?php echo esc_html( get_sub_field( 'foot3' ) ); ?></p>
								</div>
							</section>
							<section id="custom_html-2" class="widget_text footerbar widget widget_custom_html">
								<div class="textwidget custom-html-widget">
									<div class="lastline"><?php echo esc_html( get_sub_field( 'disclaimer' ) ); ?></div>
								</div>
							</section>
							<?php
						}
					}
					?>
					<a class="returntop" href="#site-header" style="							background-image:url('data:image/png;base64,')">RETURN TO TOP</a>
				</div>
			</div><!-- .site-info -->
		</div>
	</footer><!-- #colophon -->
<?php wp_footer(); ?>
</body>
</html>
